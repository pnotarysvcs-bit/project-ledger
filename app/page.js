...
const displayStatus = (value) => value === 'future' ? '' : value;
const sectionOrder = { Personal: 0, Business: 1, Streaming: 2 };
...
function applyFilters(rows, filters) {
  return rows.filter((bill) => includes(bill.payee, filters.bill)
    ...
    && exact(displayStatus(bill.status), filters.status));
}
...
async function addBill(data) {
  ...
  if (!name || !category || !account) throw new Error('Bill name, Category, and Account are required.');
  if (invalidCategory(category)) throw new Error('Business and Personal are Types, not Categories. Choose a bill category.');
  if (!/^\d{4}-\d{2}-\d{2}$/.test(dueDate)) throw new Error('A valid Due Date is required.');
  ...
  if (!['Personal', 'Streaming', 'Business'].includes(billType)) throw new Error('Choose a valid Type.');
  ...
  if (!billId) throw new Error('Bill creation was not confirmed by the database.');
  await supabaseRequest('ledger_bill_months', {
    method: 'POST',
    body: {
      bill_id: billId,
      month: `${month}-01`,
  ...
  returnTo(month, 'Bill added.');
}

async function editBill(data) {
  'use server';
  const id = String(data.get('id') ?? '');
  let occurrenceId = String(data.get('occurrenceId') ?? '');
  const month = normalizeLedgerMonth(String(data.get('month')));
  const budgetRaw = String(data.get('budget') ?? '').trim();
  const actualRaw = String(data.get('actualAmount') ?? '').trim();
  const budget = budgetRaw === '' ? null : Number(budgetRaw);
  ...
  const account = String(data.get('account')).trim().toUpperCase();
  const category = String(data.get('category') ?? '').trim();
  const existingBillRows = await supabaseRequest(`ledger_bills?select=category&id=eq.${encodeURIComponent(id)}`);
  const preservedCategory = String(existingBillRows?.[0]?.category ?? '').trim();
  const categoryForUpdate = category || preservedCategory;
  if (!categoryForUpdate) throw new Error('Category is required.');
  if (invalidCategory(categoryForUpdate)) throw new Error('Business and Personal are Types, not Categories. Choose a bill category.');
  const billType = account.startsWith('TCUB') ? 'Business' : account.startsWith('TCU') ? 'Personal' : String(data.get('type')).trim();
  const dueDate = String(data.get('nextDue') ?? '');
  if (!/^\d{4}-\d{2}-\d{2}$/.test(dueDate)) throw new Error('A valid Due Date is required.');
  await supabaseRequest(`ledger_bills?id=eq.${encodeURIComponent(id)}`, { method: 'PATCH', body: { bill_name: String(data.get('name')).trim(), bill_type: billType, category: categoryForUpdate, account, frequency: String(data.get('frequency')) } });
  if (!occurrenceId) {
  ...
  if (!occurrenceId) {
    const created = await supabaseRequest('ledger_bill_months?select=id', {
      method: 'POST',
      headers: { Prefer: 'return=representation' },
      body: { bill_id: id, month: `${month}-01`, status: null, occurrence_budget_amount: budget, actual_amount: actualAmount, due_date: dueDate, installment_key: dueDate, migration_incomplete: false },
    });
    occurrenceId = created?.[0]?.id ?? '';
    if (!occurrenceId) throw new Error('Bill occurrence creation was not confirmed by the database.');
  } else {
...
export default async function BillsPage({ searchParams }) {
  ...
  const filteredRows = applyFilters(rows, filters).sort((a, b) => String(a.nextDue ?? '').localeCompare(String(b.nextDue ?? '')) || a.payee.localeCompare(b.payee));
  ...
  return <>
    ...
    {params?.add === '1' && <section className="panel add-bill-panel" id="add-bill"><header><strong>Add Bill</strong><Link href={`/?month=${selectedMonth}${filterSuffix}`}>Cancel</Link></header><form action={addBill} className="add-bill-form"><input type="hidden" name="month" value={selectedMonth}/><label>Bill Name<input name="name" required/></label><label>Type<select name="type" defaultValue="Personal"><option>Personal</option><option>Business</option><option>Streaming</option></select></label><label>Category<input name="category" required placeholder="e.g. Utilities"/></label><label>Account<input name="account" required placeholder="TCU / TCUB / other"/></label><label>Budget Amount<input name="budget" type="number" min="0" step="0.01"/></label><label>Actual Bill Amount<input name="actualAmount" type="number" min="0" step="0.01"/></label><label>Frequency<select name="frequency" defaultValue="monthly">{['monthly','bi-weekly','quarterly','annual','one-time'].map((value) => <option key={value}>{value}</option>)}</select></label><label>Due Date<input name="nextDue" type="date" required/></label><label className="add-bill-notes">Notes<input name="notes"/></label><div className="add-bill-actions"><button type="submit">Save Bill</button><Link className="button ghost" href={`/?month=${selectedMonth}${filterSuffix}`}>Cancel</Link></div></form></section>}
    ...
    {groupedBills.map(({ type, bills }) => <section className="panel" key={type}><header><strong>{type} Bills</strong><span>{bills.length} {bills.length === 1 ? 'occurrence' : 'occurrences'}</span></header><div className="table-wrap"><table><thead><tr><th>Bill</th><th>Type</th><th>Category</th><th>Account</th><th>Budget</th><th>Actual</th><th>Due Date</th><th>Status</th><th>Actions</th></tr></thead><tbody>{bills.flatMap((bill) => {
      ...
      const statusValue = displayStatus(bill.status);
      const row = editing ? <tr key={bill.rowKey} id={rowAnchor} className="editing-row">
        ...
        <td>{statusValue && <span className={`status ${bill.status}`}>{statusValue}</span>}</td>
        <td><div className="row-actions"><button form={editFormId} type="submit">Save</button><Link className="button ghost" scroll={false} href={`/?month=${selectedMonth}${filterSuffix}`}>Cancel</Link></div></td>
      </tr> : <tr key={bill.rowKey} id={rowAnchor}><td><b>{bill.payee}</b>{bill.frequency === 'bi-weekly' && <small>Bi-weekly installment</small>}{bill.submitted > 0 && <small>Paid {money.format(bill.submitted)} · Remaining {money.format(bill.remaining ?? 0)}{bill.credit > 0 ? ` · Credit ${money.format(bill.credit)}` : ''}</small>}</td><td>{bill.type}</td><td>{displayCategory(bill.category)}</td><td>{bill.account}</td><td>{bill.budget === null ? 'Enter amount' : money.format(bill.budget)}</td><td>{bill.actualAmount === null ? '—' : money.format(bill.actualAmount)}</td><td>{displayDate(bill.nextDue)}</td><td>{statusValue && <span className={`status ${bill.status}`}>{statusValue}</span>}</td><td><div className="row-actions"><form action={submitBill}><input type="hidden" name="id" value={bill.id}/><input type="hidden" name="occurrenceId" value={bill.occurrenceId ?? ''}/><input type="hidden" name="dueDate" value={bill.nextDue}/><input type="hidden" name="month" value={selectedMonth}/><input type="hidden" name="rowKey" value={bill.rowKey}/><input type="hidden" name="returnQuery" value={returnQuery}/><button type="submit" disabled={bill.status === 'submitted' || bill.effectiveAmount === null || !bill.occurrenceId}>Submit</button></form><Link className={`button partial ${bill.effectiveAmount === null || !bill.occurrenceId ? 'disabled' : ''}`} aria-disabled={bill.effectiveAmount === null || !bill.occurrenceId} href={bill.effectiveAmount === null || !bill.occurrenceId ? `/?month=${selectedMonth}${filterSuffix}#${rowAnchor}` : `/?month=${selectedMonth}${filterSuffix}&partial=${encodeURIComponent(bill.rowKey)}#${rowAnchor}`}>Partial</Link><Link className="button ghost" scroll={false} href={`/?month=${selectedMonth}${filterSuffix}&edit=${encodeURIComponent(bill.rowKey)}`}>Edit</Link><form action={archiveBill}><input type="hidden" name="id" value={bill.id}/><input type="hidden" name="month" value={selectedMonth}/><input type="hidden" name="rowKey" value={bill.rowKey}/><input type="hidden" name="returnQuery" value={returnQuery}/><ConfirmButton className="ghost danger" message={`Archive ${bill.payee}?`}>Archive</ConfirmButton></form></div></td></tr>;
