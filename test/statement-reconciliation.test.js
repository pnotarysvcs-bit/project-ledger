...
test('detects Capital One abbreviated cross-year statement period', () => assert.deepEqual(detectStatementPeriod('Dec 22, 2025 - Jan 21, 2026 | 31 days in Billing Cycle'), { start: '2025-12-22', end: '2026-01-21', detectedMonth: '2026-01', confidence: 'confirmation-required', spansMonths: true }));
test('detects Capital One abbreviated same-year statement period', () => assert.deepEqual(detectStatementPeriod('Jun 21, 2026 - Jul 21, 2026 | 31 days in Billing Cycle'), { start: '2026-06-21', end: '2026-07-21', detectedMonth: '2026-07', confidence: 'confirmation-required', spansMonths: true }));
...
test('normalizes compact processor descriptors before fuzzy scoring', () => assert.equal(normalizePayee('Wci*WciOfMissouri'), 'wci of missouri'));
...
test('extracts Capital One named-month transaction rows', () => { const rows = extractTransactions('Jan 2   Jan 3   MYFICO.COM   $39.95', 2026, 1); assert.equal(rows.length, 1); assert.equal(rows[0].date, '2026-01-02'); assert.equal(rows[0].amount, 39.95); });
test('resolves Capital One December transaction into prior year for January cycle', () => { const rows = extractTransactions('Dec 29   Dec 30   TEST MERCHANT   $25.00', 2026, 1); assert.equal(rows.length, 1); assert.equal(rows[0].date, '2025-12-29'); });
test('uses first monetary transaction column and ignores later running balance', () => { const rows = extractTransactions('06/05 TEST MERCHANT 12.34 456.78', 2026); assert.equal(rows.length, 1); assert.equal(rows[0].amount, 12.34); });
