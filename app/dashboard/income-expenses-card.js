...
export default async function IncomeExpensesCard({ summary = {}, selectedMonth = '' }) {
  let income = null;
  let error = null;
  try {
  ...
  return (
    <article className="widget glance-card" aria-label="Monthly income versus expenses">
      ...
      <div className="glance-grid">
        <div className="glance-item blue"><small>Household Income</small><strong>{money.format(net.income)}</strong><span>Payroll {money.format(net.payrollIncome)} · notary {money.format(net.notarySupport)} · other {money.format(net.otherFunding)}</span></div>
