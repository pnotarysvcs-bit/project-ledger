...
async function updatePayment(data) {
  ...
  if (!paymentId || !id) throw new Error('Payment id and bill id are required.');
  if (!Number.isFinite(amount) || amount <= 0) throw new Error('Payment must be positive.');
  if (!/^\d{4}-\d{2}-\d{2}$/.test(paymentDate)) throw new Error('A valid payment date is required.');
  if (!fundingAccount) throw new Error('Funding account is required.');
  await supabaseRequest(`ledger_bill_payments?id=eq.${encodeURIComponent(paymentId)}&bill_id=eq.${encodeURIComponent(id)}&payment_month=eq.${month}-01`, { method: 'PATCH', body: { amount, payment_date: paymentDate, funding_account: fundingAccount, notes: notes || null } });
