...
export function buildFinancialGoals({
  rows = [],
  emergencyFundSaved = 0,
  oneMonthAheadSaved = 0,
  freedMonthlyCash = 0,
} = {}) {
  ...
  let currentPriority;
  if (actuals.missingActualCount > 0) {
    currentPriority = {
      id: 'complete-actuals',
      name: 'Complete Actual Expenses',
      status: 'needs-data',
      remaining: null,
      detail: `${actuals.missingActualCount} bill${actuals.missingActualCount === 1 ? '' : 's'} still need actual amounts.`,
    };
  } else if (expenseGap > 0) {
    currentPriority = {
      id: 'expense-target',
      name: 'Reduce Actual Monthly Expenses',
      status: 'active',
      remaining: expenseGap,
      detail: 'Bring actual monthly expenses down to the operating target.',
    };
    ...
    currentPriority = {
      ...
      detail: 'First cash-reserve milestone before building the one-month-ahead buffer.',
    };
  } else if (monthAheadRemaining > 0) {
    currentPriority = {
      id: 'one-month-ahead',
      name: 'Get One Month Ahead',
      status: 'active',
      remaining: monthAheadRemaining,
      detail: 'Build one full month of operating expenses in advance.',
    };
  } else {
    currentPriority = {
      id: 'next-payoff',
      name: 'Choose Next Payoff Target',
      status: 'ready',
      remaining: null,
      detail: 'The runway milestones are funded; redirect the rolling cash to the next obligation.',
    };
  }

  return {
    actualExpenses: {
      current: actuals.actual,
      target: MONTHLY_ACTUAL_EXPENSE_TARGET,
      remainingToCut: expenseGap,
      complete: actuals.missingActualCount === 0 && expenseGap === 0,
      missingActualCount: actuals.missingActualCount,
    },
    emergencyFund: {
      current: emergencySaved,
      target: EMERGENCY_FUND_TARGET,
      remaining: emergencyRemaining,
      complete: emergencyRemaining === 0,
    },
    oneMonthAhead: {
      current: monthAheadSaved,
      target: MONTHLY_ACTUAL_EXPENSE_TARGET,
      remaining: monthAheadRemaining,
      complete: monthAheadRemaining === 0,
    },
